...
PROJECTS_INDEX = WORKSPACE / "data/auto-knowledge-projects.json"  # 全局项目索引

# 质量阈值（优化：只记录高价值项目，避免重复模板）
MIN_SCORE = 4.0  # 只记录值得创建 Skill 的项目（≥4.0）
MIN_STARS = 500
...
def main():
    ...
    for project in projects[:5]:  # 只处理前5个
        ...
        if score >= MIN_SCORE:
            ...
            # 检查是否需要更新（项目新或 stars 变化）
            should_save = True
            if project_name in projects_index:
                old_stars = projects_index[project_name].get('stars', 0)
                if old_stars == project.get('stars', 0):
                    should_save = False
                    log(f"  ⏭️ 跳过（无变化）: {project_name}")
                else:
                    log(f"  🔄 更新（stars: {old_stars} → {project.get('stars', 0)}）: {project_name}")
            
            if should_save:
                # 保存笔记到统一目录（不按日期分文件夹）
                note_name = f"{project_name}.md"
                note_path = NOTES_DIR / "high-value-projects"
                ...
                save_projects_index(projects_index)
