...
## 🧠 Myco 项目研究（2026-04-14）
→ 详见 `memory/2026-04-14.md`
- 21K+ 行 Python 代码，26 维免疫系统（L0-L25）
- 知识排泄机制 + 引擎规则自改
- 结论：实现完整，非空壳。feZ 问是否深入具体模块

## 🚀 Hermes Agent vs OpenClaw 评估（2026-04-15）
→ 详见 `memory/2026-04-15-HERMES-AGENT-RESEARCH.md`
- NousResearch/hermes-agent：⭐85,908 | 自进化飞轮 + Progressive Disclosure Skills
- **不建议替换 OpenClaw**（176 Skills 迁移成本高、双通道已稳定、八字等自写 skills 要重写）
- 有迁移工具：`hermes claw migrate --dry-run`
- Skills 格式基本兼容 agentskills.io 标准
...
## 🧠 SOUL.md + TalentMind 内化 (2026-04-09)

### SOUL.md 三层升级
→ 详见 `memory/evolution/javers-distillation-plan.md`

**新增内容**：
- **Layer -1**：冲突解决协议（最高优先级）— 5条裁决规则解决 Layer 规则冲突
- **Layer 2.5**：javers 声音 — 性格显式4规则（直接给判断/亮立场/说不知道/有情绪）
- **场景路由表**：6种触发条件 → 自动加载 talent-mind 三层操作系统

**commit**：`3bff7c4`

### TalentMind 现状评估
- 内容好（方法论扎实），但用不起来——定位是"教学材料"而非"实用工具"
- 内化方案：在 SOUL.md 场景路由表写入6种触发条件，自动加载三层操作系统
...
### 5个核心脚本并行+增量算法升级
→ 详见 `docs/programming-learning/daily/2026-04-08.md`

| 脚本 | 优化算法 | 性能提升 |
|------|---------|---------|
| access_router.py | asyncio并行竞争（首个成功即返回）| 延迟↓60% |
| session-miner.py | 增量checkpoint | 跳过无变化会话 |
| wiki-query.py | Snippet预计算缓存 | IO↓80% |
| ecology-monitor.py | Manifest缓存+ThreadPool并行stat | stat↓90% |
| lobster_parallel.py | DAG并行执行引擎 | 吞吐↑2-5x |

**commit**：`605020b`（web-access子模块：`66b841c`）

---



## ⚠️ Git Push 待完成（2026-04-15）
- 原计划：创建 orphan branch `main-clean` 推送，去除 secret_patterns.py 中的 Slack token
- 状态：**未完成**，配置恢复打断了流程
- 当前状态：secret 文件仍在原位置，仓库无变化
- 下次操作：完成 push 后再考虑重新配置 npm Gateway
