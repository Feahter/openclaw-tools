...
description: >
  ...
  触发词：画图/生成图/做个图/架构图/流程图/可视化/generate diagram/draw diagram/visualize。
  支持 14 种图类型（架构/数据流/序列/UML/Agent/Memory）+ 7 种视觉风格。
  OpenClaw 专属模式：输入"OpenClaw 架构"直接生成 feZ 的双 Gateway + Agent 系统图。
---

# Fireworks Tech Graph

用自然语言描述 → 产出级 SVG + 1920px PNG。
...
| 风格 | 示例图 |
|------|--------|
| Style 6 Claude Official | `assets/samples/sample-style6-claude.png` — 暖奶油色，最适合 OpenClaw |
| Style 2 Dark Terminal | `assets/samples/sample-style2-dark.png` — 暗黑极客风 |
| Style 5 Glass | `assets/samples/sample-style5-glass.png` — Multi-Agent 协作 |
| Style 3 Blueprint | `assets/samples/sample-style3-blueprint.png` — 微服务架构 |

> 生成前读对应的 sample 图片，建立视觉预期。

---

## 执行流程（4步）

> **OpenClaw 专属模板** → `references/openclaw-patterns.md`
> **各 type 最小可用 JSON** → `references/recipes.md`
> **完整 layout 规范** → `references/diagram-type-layouts.md`
> **风格颜色 tokens** → `references/style-guides/index.md`
> **SVG 生成方法** → `references/svg-generation.md`
> **错误处理** → `references/error-prevention.md`

### Step 1 · 分类（系统A直觉）

**直觉判断**：
- "用户→服务→数据" → architecture
- "LLM/Tool/Memory/Agent" → agent / memory
- "步骤/流程/决策" → flowchart
- "A和B对比" → comparison
- "时间顺序/接口调用" → sequence
...
| 场景 | 风格 |
|------|------|
| OpenClaw 系统 | **Style 6 Claude Official** |
| GitHub README | Style 2 Dark Terminal |
| 架构文档 | Style 3 Blueprint |
| Multi-Agent 协作 | Style 5 Glass |
| 对比矩阵 | Style 4 Notion Clean |
| 博客/文档 | Style 1 Flat Icon |
...
| LLM/Model/Agent | rect（double border）|
...
**OpenClaw 专属**（最快路径）：
```bash
python3 scripts/generate-from-template.py agent /tmp/diagram.svg '<JSON>'
```
JSON 直接抄 `references/openclaw-patterns.md` 或 `references/recipes.md` 中的模板。
...
```bash
rsvg-convert output.svg -o /dev/null 2>&1 && echo "✓ Valid" || echo "✗ Error"
...
| OpenClaw Agent | Agent Core 架构图（Style 6）|
| OpenClaw 双 Gateway | QClaw 28789 + npm 18789 双通道图（Style 6）|
| Skill 系统架构 | Skill orchestrator + specialists 分层图（Style 5）|
| OpenClaw Memory | Session/Daily/Long-term/Skill 四层记忆图（Style 3）|
| OpenClaw Tool Call | LLM → Tool Selector → MCP → Execution → LLM 循环（Style 2）|

---

## OpenClaw Agent 模板 JSON（Style 6 — 直接可用）

```json
{"style":6,"title":"OpenClaw Agent Architecture","subtitle":"AI Agent System",
 "nodes":[
  ...
  {"id":"skills","kind":"rect","x":280,"y":360,"width":200,"height":70,"label":"Skills System","sublabel":"Tool · Workflow","type_label":"EXTENSION","fill":"#e8f0fc","stroke":"#8c6f5a"},
  ...
  {"id":"output","kind":"rect","x":760,"y":220,"width":160,"height":70,"label":"Output","sublabel":"Response","type_label":"OUTPUT","fill":"#fffcf7","stroke":"#ded0c3"}
 ],
 "arrows":[
 ...
 "legend":[{"flow":"control","label":"Control"},{"flow":"read","label":"Read"},{"flow":"write","label":"Write"},{"flow":"feedback","label":"Feedback"},{"flow":"data","label":"Data"}],
...
| 类型 | Style | 用途 |
|------|-------|------|
| Agent/Memory | Style 6 | OpenClaw / Anthropic 风格 |
| Multi-Agent | Style 5 | Glassmorphism 协作图 |
| Memory 分层 | Style 3 | Blueprint 蓝图风格 |
| Tool Call 流程 | Style 2 | Dark Terminal 极客风 |
| RAG Pipeline | Style 1 | 通用流程图 |
| 系统对比 | Style 4 | Notion Clean 矩阵 |
>>>>>>> Stashed changes
