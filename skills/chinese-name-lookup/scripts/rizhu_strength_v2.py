...
引用体系：
- 《千里命稿》韦千里（民国）：格局法+调候优先
- 《子平真诠》沈孝瞻（清）：月令格局为经
- 《滴天髓阐微》任铁樵（清）：气势平衡+动态辩证

两套方法：
1. 权重法（千里命稿+子平真诠）：月令40%+通根30%+临宫15%+比劫20%+印星10%
2. 计数法（任铁樵·滴天髓阐微·简化版）：全藏干五行数量统计
...
def get_strength_by_count(bazi: Dict[str, Any]) -> Dict[str, Any]:
    ...
    方法来源：任铁樵《滴天髓阐微》"气势平衡"思维
    - 任铁樵（清）：认为日主强弱非机械公式，应看五行整体气势
    - 核心："得时、得地、得势"的综合气机，而非简单数个数
    - 注：本代码为简化版，完整版需结合"君赖臣生"等动态辩证理论
    - 参考：网络平台（卜知排盘、好赞易学等）的简化计数法
