...
**缺失出生时间时的处理**：
- 如果用户只提供姓名 → 调用 `analyze_existing_name()` → 输出基础名字分析（含姓氏五行、名字用字分析）
- **必须明确询问**：回复模板 →「好的，张先生/女士，完整命理分析需要出生时间。请问：出生年份（阳历/农历）、出生月份、具体日期、以及大约几点？（只需要大概时段如'上午/下午'也可，我会据此推算时柱）」
- 不得捏造出生时间或生成不完整的八字命盘
- 如果用户只给预产期（如"2026年6月"）→ 视为缺少具体出生日，可先做生肖/五行初步分析，同时告知"预产期只知月份，建议您等宝宝出生后提供准确日期再生成完整命盘"
...
> ⚠️ **使用说明**：宜用偏旁表示对该生肖有吉利的暗示；忌用偏旁表示可能带来冲/刑/害的暗示。**优先级规则**：
> - 若一个字同时出现在宜用和忌用列（如牛含"艹"），以**忌用为主**——此时该字对该生肖有潜在冲/害风险，不推荐作为起名用字。
> - 部分"忌用"列填入的是**地支冲/刑关系**（如兔忌"子午"冲），而非偏旁——这表示属该地支的年份/生肖与命主存在冲刑，名字中应避免对应偏旁。
> - 最终以 `zodiac_preferences.py` 的实际代码逻辑为准，本表仅作参考速查。

| 生肖 | 宜用偏旁 | 忌用偏旁（含冲刑地支） |
|------|---------|---------|
| 鼠 | 米、豆、鱼、虫、木、人、宀、艹、金 | 火、马、午、羊、田、日、辶 |
| 牛 | 田、禾、鸟、蛇、虫、马、钅 | 马、羊~~、艹~~、日、火 |
| 虎 | 马、戌、犬、王、大、君、木、月 | 申、巳、猴、刀、戈、己 |
| 兔 | 艹、禾、木、月、鸟、虫、人、宀 | 酉（冲卯）、辰（害卯）、子（冲卯）、午（冲卯）、鹰 |
| 龙 | 辰、龙、申、酉、氵、扌、三、君 | 寅（冲申）、卯（六合）、戌（刑辰） |
| 蛇 | 艹、虫、豆、鱼、禾、木、衣、巾 | 亥（冲巳）、猪（害巳）、虎（刑巳）、猴（冲巳） |
| 马 | 艹、龙、蛇、衣、巾、忄、玆、辶 | 牛（害午）、子（冲午） |
| 羊 | 艹、禾、三合、手、君、木、月 | 丑（冲未）、牛（害未）、鼠（冲未） |
| 猴 | 申、酉、辰、子、三合、大、君 | 寅（冲申）、卯（冲申） |
| 鸡 | 酉、丑、巳、谷、米、艹、虫 | 卯（冲酉） |
| 狗 | 寅、卯、戌、马、君、艹、三合 | 辰（冲戌）、巳（害戌）、午（刑戌） |
| 猪 | 亥、卯、未、艹、米、豆、禾、忄 | 巳（冲亥）、蛇（害亥） |
...
| 错误类型 | 处理方式 |
|---------|---------|
| API Key 无效/未配置 | 自动切换本地引擎，喜用神为估算值，并在报告中注明"喜用神为本地估算，如需精确喜用神请配置八字API密钥" |
| 笔画数表中无该字 | 使用偏旁推断，默认笔画数8，不拒绝处理 |
| 候选字不足3个 | 放宽生肖要求，直至凑满3个 |
| 农历转换超范围 | 提示超出1900-2100年范围，询问是否改用相近年份估算 |
| 用户只给预产期月份（如"2026年6月"） | 输出生肖初步分析 + 姓名候选，报告中注明"请在宝宝出生后提供准确日期以生成完整命盘" |
| 用户提供出生日期但无具体时辰 | 使用起名场景默认规则（按卯时/酉时估算），报告中注明"时柱为估算，建议提供准确时辰" |
