...
description: >
  GitHub 项目代码分析专家。当用户想理解代码架构、技术栈、依赖关系时触发。
  触发条件（满足任一）：
  - "代码怎么组织的"、"看看这个代码库"、"项目结构"、"怎么用"
  - "帮我改这个项目"、"接手这个代码"、"看看依赖"
  - 已 clone 的项目路径，要求分析实现细节
  关键词：代码分析 / 架构 / 目录结构 / 依赖 / 入口文件 / 模块
  不触发：技术选型问"值不值得用"、生态问"和 XX 比怎么样"、纯学习原理 → 用 skill-repo-research 统一入口。
---
