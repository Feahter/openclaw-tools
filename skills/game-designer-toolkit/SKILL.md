...
> 本工具覆盖**通用游戏设计**。垂直领域：文字游戏→`text-game-dev`，Web开发→`game-engine`，设计思维→`game-design-philosophy`
...
## Game Design Skills 导航

本工具覆盖**通用游戏设计**。根据你的具体场景，可能需要配合以下专用 Skills：

| 场景 | 推荐 Skill | 用途 |
|------|-----------|------|
| **文字/视觉小说** | `text-game-dev` | 离线文字游戏专用：玩法、引擎选择、美术、存档、UI、结局 |
| **Web/HTML5游戏** | `game-engine` | 2D/3D Web游戏开发：Canvas、WebGL、Phaser、Three.js |
| **设计思维/哲学** | `game-design-philosophy` | 设计理念、玩家体验思维、元技能 |

**推荐工作流**：
```
设计思维 (philosophy) → GDD/系统设计 (toolkit) → 执行实现 (engine/text-game-dev)
```

### 垂直领域 Skills

- **`text-game-dev`**：离线版文字游戏（视觉小说、文字冒险）
  - 核心玩法、叙事分支、引擎选择（Ren'Py/Twine/VN Maker）
  - 美术资源、音效音乐、存档系统、UI/UX、多结局

- **`game-engine`**：Web 游戏引擎开发
  - HTML5 Canvas、WebGL、JavaScript
  - 2D/3D 渲染、物理、碰撞检测、多人游戏
